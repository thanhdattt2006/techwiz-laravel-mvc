...
export default function UserDashboard() {
  ...
  // Read saved medical profile from storage or use defaults
  const [medicalProfile] = useState(() => {
    try {
    ...
    return {
      ...
      icePrimary: { name: 'Dr. Arthur Vance', relation: 'Spouse', phone: '0918-999-888' },
  ...
  const handleTriggerSOS = async () => {
    const allergyText =
      medicalProfile.allergies && medicalProfile.allergies.length > 0
        ? `\n• Critical Allergies: ${medicalProfile.allergies.join(', ')}`
        : '';
    const bloodText = `\n• Blood Type: ${medicalProfile.bloodGroup || 'O+'}`;
    const iceText = medicalProfile.icePrimary
      ? `\n• ICE Emergency Contact: ${medicalProfile.icePrimary.name} (${medicalProfile.icePrimary.phone})`
      : '';

    const confirmed = await showConfirm({
      title: 'Confirm Emergency SOS Broadcast',
      message: `Transmit GPS coordinates and medical dossier to Chicago Central Emergency Dispatch?${bloodText}${allergyText}${iceText}`,
      type: 'danger',
    ...
    if (confirmed) {
      ...
      showAlert({
        ...
        message:
          'Operator has received your coordinates and attached clinical dossier. Nearest unit AMB-CHI-102 (Ford Transit ICU) is en route. ETA: 5 mins.',
        type: 'success',
        confirmText: false,
        autoCloseMs: 2500,
      });
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 sm:p-8 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/user/history"
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 text-xs font-bold hover:bg-purple-100 transition"
          >
      ...
      {/* Emergency Medical Dossier Snapshot Widget */}
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-red-50 text-[#DC3545] border border-red-200 flex items-center justify-center font-black text-base shrink-0">
            {medicalProfile.bloodGroup || 'O+'}
          </div>
          <div className="space-y-1 text-xs">
            <div className="flex items-center gap-2">
              <span className="font-bold text-[#1F2A37]">Emergency Clinical Dossier</span>
              <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                Transponder Linked
              </span>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-[#6B7785]">
              <span>Blood: <strong className="text-[#1F2A37]">{medicalProfile.bloodGroup || 'O+'}</strong></span>
              <span>•</span>
              <span>
                Allergies:{' '}
                <strong className="text-red-600">
                  {medicalProfile.allergies?.length > 0 ? medicalProfile.allergies.join(', ') : 'None Reported'}
                </strong>
              </span>
              <span>•</span>
              <span>
                ...
                <strong className="text-[#1F2A37]">
                  {medicalProfile.icePrimary?.name} ({medicalProfile.icePrimary?.phone})
                </strong>
              </span>
            </div>
          </div>
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-50 border border-blue-200 text-[#0B6EFD] text-xs font-bold hover:bg-blue-100 transition shrink-0"
        >
          <ShieldCheck className="w-4 h-4 text-[#0B6EFD]" />
          <span>Manage Clinical Dossier</span>
      ...
      <div className="bg-gradient-to-br from-red-50 to-white border-2 border-red-200 rounded-3xl p-6 sm:p-10 shadow-md text-center space-y-6">
        ...
        {sosActive && (
          <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs max-w-xl mx-auto shadow-xs space-y-3">
            ...
            {/* Automatically Attached Medical Warning Tags */}
            <div className="pt-2 border-t border-emerald-200/80 text-left space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-900 block">
                Attached Medical Alert Dossier:
              </span>
              <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                <span className="px-2 py-0.5 rounded bg-red-100 text-[#DC3545] font-bold border border-red-200">
                  Blood {medicalProfile.bloodGroup || 'O+'}
                </span>
                {medicalProfile.allergies?.map((all, i) => (
                  <span key={i} className="px-2 py-0.5 rounded bg-amber-100 text-amber-900 font-semibold border border-amber-200">
                    Allergy: {all}
                  </span>
                ))}
                {medicalProfile.conditions?.map((cond, i) => (
                  <span key={i} className="px-2 py-0.5 rounded bg-blue-100 text-[#0B6EFD] font-medium border border-blue-200">
                    {cond}
                  </span>
                ))}
                <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 font-medium">
                  ICE: {medicalProfile.icePrimary?.name}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-[#E2E8F0]">
          ...
          <Link
            to="/user/history"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0B6EFD] hover:underline"
          >
            <span>View All Records (4)</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
