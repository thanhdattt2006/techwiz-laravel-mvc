...
export default function ForgotPasswordPage() {
  ...
  const handleResendOtp = () => {
    ...
    showAlert({
      ...
      message: 'New OTP code (123456) sent. You may verify below.',
      type: 'info',
      confirmText: false,
      autoCloseMs: 1800,
    });
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex items-center justify-between pb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black tracking-tight text-[#1F2A37]">
              Life<span className="text-[#0B6EFD]">Link</span> Recovery
            </span>
          </div>
        </div>

        {/* Step Progression Indicator */}
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-4 flex items-center justify-between shadow-xs text-xs">
          <div className="flex items-center gap-2">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${
                step >= 1 ? 'bg-[#0B6EFD] text-white' : 'bg-slate-100 text-[#6B7785]'
              }`}
            >
              1
            </span>
            <span className={`font-semibold ${step === 1 ? 'text-[#0B6EFD]' : 'text-[#1F2A37]'}`}>
              Request OTP
            </span>
          </div>
          ...
          <div className="w-12 h-0.5 bg-[#E2E8F0]">
            <div
              className={`h-full bg-[#0B6EFD] transition-all duration-300 ${
                step === 2 ? 'w-full' : 'w-0'
              }`}
            ></div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs ${
                step === 2 ? 'bg-[#0B6EFD] text-white' : 'bg-slate-100 text-[#6B7785]'
              }`}
            >
              2
            </span>
            <span className={`font-semibold ${step === 2 ? 'text-[#0B6EFD]' : 'text-[#6B7785]'}`}>
              Reset Password
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="text-center space-y-2">
            <div className="w-12 h-12 rounded-2xl bg-blue-50 text-[#0B6EFD] flex items-center justify-center mx-auto mb-2">
              <KeyRound className="w-6 h-6" />
            </div>
            <h1 className="text-xl font-black text-[#1F2A37]">
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#1F2A37] mb-1.5">
                  Email Address or Username <span className="text-[#DC3545]">*</span>
                </label>
              ...
              <div className="flex items-center justify-between text-[11px] text-[#6B7785] pt-1">
                <span>Test with demo account:</span>
                <button
                  type="button"
                  onClick={() => setIdentity('user@gmail.com')}
                  className="text-[#0B6EFD] font-bold hover:underline cursor-pointer"
                >
                  user@gmail.com
                </button>
              </div>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              ...
              <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-[#0B6EFD] font-bold hover:underline cursor-pointer"
                >
                  Auto-fill
                </button>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#1F2A37] mb-1.5">
                  6-Digit Verification Code <span className="text-[#DC3545]">*</span>
                </label>
                <input
                  type="text"
                  maxLength={6}
                  ...
                  className="w-full px-4 py-2.5 text-center font-mono text-base tracking-widest bg-[#F5F8FC] border border-[#E2E8F0] rounded-xl text-[#1F2A37] focus:outline-none focus:border-[#0B6EFD] transition"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#1F2A37] mb-1.5">
                  New Account Password <span className="text-[#DC3545]">*</span>
                </label>
                <div className="relative">
                  <Lock className="w-4 h-4 text-[#6B7785] absolute left-3 top-3.5" />
                  <input
                    type="password"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-[#1F2A37] mb-1.5">
                  Confirm New Password <span className="text-[#DC3545]">*</span>
                </label>
                <div className="relative">
                  <CheckCircle2 className="w-4 h-4 text-[#6B7785] absolute left-3 top-3.5" />
                  <input
                    type="password"
                    required
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                <button
                  ...
                  className="text-[#6B7785] hover:text-[#1F2A37] font-semibold cursor-pointer"
                >
                  Change Email / User
                </button>
                ...
                <button
                  ...
                  className={`flex items-center gap-1 font-bold ${
                    countdown > 0
                      ? 'text-[#6B7785] cursor-not-allowed'
                      : 'text-[#0B6EFD] hover:underline cursor-pointer'
                  }`}
                >
          ...
          {/* Bottom helper */}
          <div className="pt-4 border-t border-[#E2E8F0] text-center text-xs text-[#6B7785]">
